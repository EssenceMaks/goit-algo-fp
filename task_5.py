...
def draw_tree(tree_root, visited_nodes=None, title="Figure"):
    ...
    # Зберігаємо кожний крок виконання
    algorithm_type = 'depth' if 'глибину' in title.lower() else 'breadth'
    step = len([f for f in os.listdir('images') if f.startswith(f'task_5_{algorithm_type}_step')])
    plt.savefig(f'images/task_5_{algorithm_type}_step_{step}.png')
    plt.show()
    plt.close()
